...
4MA 是面向微信 **7MA 出行**小程序的 libxposed API 102 模块。当前 `0.1.10`
...
3. 首次启用后强制停止并重新打开微信，再进入 7MA 出行小程序，浏览地图、车辆详情和用车页面。
   从 `0.1.8` 或更新版本覆盖安装时，可先直接测试热重载；报告出现 `hot_reload` 后无需重启微信。
...
热重载成功时，报告会出现 `hot_reload|new generation loaded`。从不支持热重载的旧版本升级到
本版本时，旧代码仍会拒绝热重载，因此需要让微信重新加载模块一次。
