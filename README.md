...
4MA 是面向微信 **7MA 出行**小程序的 libxposed API 102 模块。当前 `0.2.2`
开始提供实验性主体功能：页面控制面板、跨运营区提示自动继续、临时锁车超时前再次开锁、
18 分 30 秒还车重借，以及车辆详情一键借车。诊断探针继续记录经过白名单过滤的运行信息。

`0.2.1` 开始将操作实现迁移到小程序函数边界：探针包装 `wx.request`、
`wx.cloud.callFunction` 与 `WeixinJSBridge.invoke`，只导出调用名、无查询参数的接口路径和参数键名，
用于确定借车、锁车、开锁与还车的稳定底层调用。Token、签名及实际参数值不会写入报告。

实机已确认 7MA AppID 为 `wx9a6a1a8407b04c5d`，页面由 XWeb Pinus 渲染。当前版本会在匹配
该 AppID 后，通过 XWeb 注入控制面板，并以只读探针读取页面路径和固定业务标记；页面点击后
的复查最短间隔为 1.5 秒。诊断数据不会回传完整页面正文。

增强功能开启后会按用户在 4MA 面板中的操作提交借车、开锁或还车。它不 hook 网络正文，不记录
...
- **启用 4MA 控制面板**：在 7MA 页面显示用车操作入口，默认开启。
- **自动继续用车**：自动处理跨运营区的“继续用车”提示，默认开启。
...
“启动守护”以点击时刻作为本次计时起点：临时锁车页面持续出现“再次开锁”达到 4 分 10 秒时
自动开锁；达到 18 分 30 秒时执行一次还车重借。车辆编号能从页面读取时自动保存；页面只显示
星号编号时，可以在 4MA 面板中填写完整编号。流程找不到预期按钮或输入框时会停止守护，避免
循环提交。
